...
class Infoproductos extends Component{
    ...
    public function render(){
        ...
        $mas_vendidos = DB::select("
                SELECT v.producto_id, a.nombre, SUM(v.cantidad) AS TotalVentas 
        ...
        $mas_comprados = DB::select('
                SELECT d.producto_id, a.nombre, SUM(d.cantidad) AS TotalCompras 
        ...
        foreach($mas_vendidos as $ven){
            $mas_vendidos_nombre[]=$ven->nombre;
        ...
        foreach($mas_comprados as $com){
            $mas_comprados_nombre[]=$com->nombre;
